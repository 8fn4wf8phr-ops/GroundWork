...
import { z } from "zod"
import { narrate, parseStructured } from "@/lib/server/agent-route"

// The Compass/Scout exchange (spec §8), shared by the interactive route
// (app/api/agents/review-jobs, called from the browser after a manual
// pull) and the scheduled-discovery cron (which calls it directly, with
// no HTTP hop). Deterministic facts come in; the model only narrates them.

// Mirrors MAX_JOBS_TO_REVIEW on the client (lib/agents/review-jobs.ts) —
// enforced server-side too, since a client-side cap doesn't bind a direct
// caller, and shared with the cron so both paths cost the same at most.
export const MAX_JOBS_PER_REVIEW = 3
...
export const ReviewProfileSchema = z.object({ targetRoles: z.array(z.string().max(200)).max(20) })
export type JobInput = z.infer<typeof ReviewJobSchema>
export type ProfileInput = z.infer<typeof ReviewProfileSchema>

export type CaseFileEntryDraft = {
  agent: "Compass" | "Scout"
  message: string
  jobId: string
...
export async function reviewJobs(
  ...
  const settled = await Promise.allSettled(jobs.slice(0, MAX_JOBS_PER_REVIEW).map((job) => reviewOneJob(client, job, profile)))
